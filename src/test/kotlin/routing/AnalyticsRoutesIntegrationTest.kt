...
class AnalyticsRoutesIntegrationTest {
    ...
    @Test
    fun `GET admin analytics searches - should return 200 with empty list when no searches exist`() = testApplication {
        // Arrange
        val fakeActivityRepo = FakeActivityRepository()
        val fakeAnalyticsRepo = FakeAnalyticsRepository()
    ...
    @Test
    fun `GET admin analytics overview - should return 200 with combined analytics`() = testApplication {
        // Arrange
        val fakeActivityRepo = FakeActivityRepository()
        val fakeAnalyticsRepo = FakeAnalyticsRepository()
        ...
        // Act
        val response = client.get("/admin/analytics/overview")

        // Assert
        assertEquals(HttpStatusCode.OK, response.status)

        val overview = response.body<Map<String, Any>>()
        assertTrue(overview.containsKey("trends"))
        assertTrue(overview.containsKey("searches"))
        assertTrue(overview.containsKey("activities"))
    }

    @Test
    fun `GET admin analytics overview - should return 200 even when no data exists`() = testApplication {
        // Arrange
        val fakeActivityRepo = FakeActivityRepository()
        val fakeAnalyticsRepo = FakeAnalyticsRepository()
